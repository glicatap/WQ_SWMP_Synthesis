...
load(here::here("Data", "QAQCd_daily", "elkcwmet_daily.RData"))
load(here::here("Data", "QAQCd_daily", "lkspomet_daily.RData"))

met <- bind_rows(list(gndcrmet = gndcrmet_daily,
                 wqbchmet = wqbchmet_daily,
                 soscmmet = soscmmet_daily,
                 niwolmet = niwolmet_daily,
                 elkcwmet = elkcwmet_daily,
                 lkspomet = lkspomet_daily),
                 .id = "station") %>% 
...
load(here::here("Data", "QAQCd_daily", "elknmwq_daily.RData"))
load(here::here("Data", "QAQCd_daily", "lksbawq_daily.RData"))

wq_daily <- bind_rows(list (gndbhwq = gndbhwq_daily,
                      wqbmpwq = wqbmpwq_daily,
                      sosvawq = sosvawq_daily,
                      niwolwq = niwolwq_daily,
                      elknmwq = elknmwq_daily,
                      lksbawq = lksbawq_daily),
                 .id = "station") %>% 
  mutate(doy = lubridate::yday(date),
         year = lubridate::year(date),
         month = lubridate::month(date))
...
```{r wq-data-15min}
gndbhwq_qc <- get(load(here::here("Data", "QAQCd_by_stn", "gndbhwq_qc.RData")))
wqbmpwq_qc <- get(load(here::here("Data", "QAQCd_by_stn", "wqbmpwq_qc.RData")))
niwolwq_qc <- get(load(here::here("Data", "QAQCd_by_stn", "niwolwq_qc.RData")))
sosvawq_qc <- get(load(here::here("Data", "QAQCd_by_stn", "sosvawq_qc.RData")))
elknmwq_qc <- get(load(here::here("Data", "QAQCd_by_stn", "elknmwq_qc.RData")))
lksbawq_qc <- get(load(here::here("Data", "QAQCd_by_stn", "lksbawq_qc.RData")))

wq_15min <- bind_rows(list (
  gndbhwq = gndbhwq_qc,
  wqbmpwq = wqbmpwq_qc,
  sosvawq = sosvawq_qc,
  niwolwq = niwolwq_qc,
  ...
  lksbawq = lksbawq_qc),
  .id = "station") %>% 
  mutate(doy = lubridate::yday(datetimestamp),
...
```{r par-plots2, eval = FALSE}
ggplot(met, aes(x = doy, y = totpar_total, col = station)) +
