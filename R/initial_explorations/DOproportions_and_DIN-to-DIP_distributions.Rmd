...
```{r}
table(dat_nut$DIN_to_DIP_censBoth)
```

Not a huge proportion having both values censored. How does that break down by station?  

```{r}
ratio_cens <- janitor::tabyl(dat_nut, station, DIN_to_DIP_censBoth) |> 
  setNames(c("station", "no", "yes")) |> 
  mutate(proportion_both_cens = round(yes / (yes + no), 4)) |> 
  arrange(desc(proportion_both_cens))

hist(ratio_cens$proportion_both_cens, breaks = 20,
     xlab = "Proportion of values, by station, where DIN and DIP are both censored",
     main = "Dual-censoring frequency")
```

Seems to be a big break at about 20% - only a few stations have more "both censored" values than that. I'll print the table for all that have more than 10% "both censored", because I'd be pretty uncomfortable just ignoring more than 10% of a dataset (even that gives me some pause).  Unfortunately these seem to be from just a handful of places:  

```{r}
ratio_cens |> 
  filter(proportion_both_cens > 0.1) |> 
  knitr::kable(caption = "Are both DIN and DIP values censored?")
```

Wells, Grand Bay, Jobos Bay, and North Carolina with >20%. Apalach and Weeks Bay add quite a few stations between 10-20%; so by ignoring this issue we're excluding all three of the northern Gulf reserves (plus Jobos, NOC, and Wells).  20 stations have at least 10% of the values qualifying as "both censored".  
...
Quick scatterplot and correlations.  

First turn anything above 200 into 200. (this is ~4% of the non-NA values.)  

`sum(dat_nut$DIN_to_DIP >= 200, na.rm = TRUE)/sum(!is.na(dat_nut$DIN_to_DIP))`

```{r}
dat_nut2 <- dat_nut |> 
  mutate(DIN_to_DIP = case_when(DIN_to_DIP >= 10000 ~ 10000,
                                .default = DIN_to_DIP + 0.0001),
         censoring = case_when(DIN_to_DIP_censBoth == 1 ~ "both",
                               DIN_to_DIP_censNum == 1 ~ "DIN",
                               DIN_to_DIP_censDenom == 1 ~ "DIP",
                               .default = "neither"))
```

```{r}
ggplot(dat_nut2) +
  geom_point(aes(x = po4f, y = DIN_to_DIP, col = censoring)) +
  scale_x_log10(labels = label_comma()) +
  scale_y_log10(labels = label_comma()) +
  annotation_logticks(sides = "bl") +
  scale_color_brewer(palette = "Paired") +
  labs(title = "DIN:DIP vs PO4",
       x = "PO4 (mg/L)",
       y = "DIN:DIP",
       color = "What is censored")


ggplot(dat_nut2) +
  geom_point(aes(x = nh4f, y = DIN_to_DIP, col = censoring)) +
  scale_x_log10(labels = label_comma()) +
  scale_y_log10(labels = label_comma()) +
  annotation_logticks(sides = "bl") +
  scale_color_brewer(palette = "Paired") +
  labs(title = "DIN:DIP vs NH4",
       x = "NH4 (mg/L)",
       y = "DIN:DIP",
       color = "What is censored")

ggplot(dat_nut2) +
  geom_point(aes(x = no23f, y = DIN_to_DIP, col = censoring)) +
  scale_x_log10(labels = label_comma()) +
  scale_y_log10(labels = label_comma()) +
  annotation_logticks(sides = "bl") +
  scale_color_brewer(palette = "Paired") +
  labs(title = "DIN:DIP vs NO23",
       x = "NO23 (mg/L)",
       y = "DIN:DIP",
       color = "What is censored")
```
