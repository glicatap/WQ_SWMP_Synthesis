...
That's not really doing very well with the higher probabilities.

What if I do just use the gaussian model? How bad would that look?  

```{r}
gnd_bam2 <- run_bam_wq(gnd, k = 12)
rhos <- gnd_bam2$rho_info
gnd_bam2 <- gnd_bam2$dat_bam
appraise(gnd_bam2)
```

I don't like the residuals vs. predictor plot. The others look really good actually.  

```{r}
summary(gnd_bam2)
```

Pretty different slope estimate, though it wasn't significant in either model. Similar R^2.  

```{r}
predictions2 <- data.frame(fitted = gnd_bam2$fitted.values,
                          dec_date = gnd_bam2$model$dec_date
)
ggplot() +
  geom_point(data = gnd, aes(x = dec_date, y = value, col = "observed")) +
...
Really doesn't do any better with the high points.  
...
I don't like the residuals vs. predictor plot, or observed vs. fitted.   
...
This seems to generally be worse. So I'm sold now on at least the `betar` distribution, rather than `gaussian`, though I still wonder if there's a better one.  
