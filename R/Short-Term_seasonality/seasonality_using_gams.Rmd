...
bhchl_ann <- annualize(bhchl_raw, chla_n) |> # note this is not grouped by station but probably should be
  mutate(mean_peak_month = as.numeric(map(peak_month, mean)),
         mean_floor_month = as.numeric(map(floor_month, mean)),
         median_peak_month = as.numeric(map(peak_month, median)),
         median_floor_month = as.numeric(map(floor_month, median)))
```
...
bhchl_ann |> 
  summarize(.by = station,
            overall_mean_peak = mean(mean_peak_month),
            overall_medianOfMedians_peak = median(median_peak_month),
            overall_medianOfMeans_peak = median(mean_peak_month),
            overall_mean_floor = mean(mean_floor_month),
            overall_medianOfMedians_floor = median(median_floor_month),
            overall_medianOfMeans_floor = median(mean_floor_month)) |> 
  knitr::kable()
```
...
```{r}
m3 <- gam(log_chla ~ s(year) + s(month) + ti(year, month),
          data = bhchl2)
summary(m3)
appraise(m3)
draw(m3)

month_smooth <- smooth_estimates(m3, smooth = "ti(month)")
ggplot(month_smooth) +
  geom_ribbon(aes(x = month, ymin = est - 2*se, ymax = est + 2*se),
              fill = "gray80") +
  geom_line(aes(x = month, y = est)) +
...
draw(m3) +
  scale_y_continuous(breaks = scales::pretty_breaks()) +
  labs(x = "year-month")
draw(m3, contour = FALSE)
```

How about another station, maybe in the great lakes?  

```{r}
chl_dat <- nut |> 
  filter(station == "lksolnut") |> 
  mutate(date = lubridate::ymd(paste(year, month, 15)),
         dec_date = lubridate::decimal_date(date),
         log_chla = log10(chla_n))
```
...
```{r}
m1 <- gam(log_chla ~ s(month, bs = "cc") + s(dec_date),
          data = chl_dat)
summary(m1)
draw(m1)
appraise(m1)
...
To interpret what's happening, make predictions from first and last years, and plot together? And/or plot the differences?  

```{r}
newdat <- data.frame(year = c(rep(min(chl$year), 12),
                              rep(max(chl$year), 12)),
                     month = c(1:12, 1:12)
) |> 
  mutate(date = lubridate::ymd(paste(year, month, 15)))
preds <- predict(m3, newdata = newdat, se.fit = TRUE)
...
Are differences bigger in some months than others? This is where it might stand out more - the gap in summer looks like it might be bigger than that in winter based on the graph above.  
...
Okay, it's subtle, but yes we do have higher changes in summer than fall/winter. Interesting. And I guess if timing was changing we'd see like a big negative change in one month and a big positive change in another month? ....only if the years weren't both "weird" ones. 

Now what to do with all this.  

I think gams may be best suited to telling us whether something about seasonality is changing, but we'd have to look at the output in other ways (maybe calculate different trends???) to know how amplitude and timing are changing.  
...
p <- ggplot(bhchl_ann) +
  ...
  geom_hline(yintercept = median(bhchl_ann$median_peak_month),
             col = "gray40",
             linetype = "dashed") +
  scale_y_continuous(breaks = scales::pretty_breaks()) +
...
p +
  geom_smooth(aes(x = year, y = median_peak_month),
              method = "loess")
