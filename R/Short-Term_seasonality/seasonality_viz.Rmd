...
This file is exported and used in `trend_visualizations.Rmd` in the slopes ~ seasonal amplitude plots.  
...
Cloern and Jassby normalized amplitude by dividing by the yearly mean value. This was because variability in amplitude increased with increasing chl concentrations. It is probably not necessary to normalize all parameters, and in fact is probably even a bad idea for the DO proportions. Some plots below show each parameter with both non-normalized and normalized amplitudes. I think it's really only chl that needs to be normalized. Both versions of amplitude for each parameter are in the exported file though.   
