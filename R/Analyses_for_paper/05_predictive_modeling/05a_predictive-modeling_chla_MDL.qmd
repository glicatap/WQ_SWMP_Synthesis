...
formula_nlme2 <- paste0("chla_trend ~ ", paste(names(dat_chl[c(5,9,13,15)]), collapse = " + "))
...
mod_chl2 <- lme(as.formula(formula_nlme2),
                data = dat_chl,
                random = ~ 1|reserve,
                method = "REML")
...
preds_df$possibly_influential[c(85)] <- 'tjros'
preds_df$possibly_influential[c(87)] <- 'tjrsb'
preds_df$possibly_influential[c(91)] <- 'welsm'
preds_df$possibly_influential[c(78)] <- 'sfbcc'
...
```{r}
cbind("station" = dat_all3$station[c(45, 87, 85, 91,78)], dat_chl[c(45, 87, 85, 91,78), c("spcond_trend", "precp_median", "precp_trend", "temp_trend")], "Pearson residual" = preds_df$resids[c(45, 87, 85, 91,78)])
```
