...
output-file: "2024-08-06 Model Outputs main"
date: today
format: 
  pdf:
    toc: true
...
The graph above and the table below indicate 3 major predictors for DO < 2 trend / hypoxia trend: NO23 trend and median precipitation, both negative; and PO4 trend, positive. Five predictors are moderate: median SpCond (negative), Chl a trend (positive), turbidity trend (negative), SpCond trend (positive), and median chl a (negative).   
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = TRUE) +
  labs(title = "Partial effect of NO23 trend on DO < 2 trend",
       x = "NO23 trend (%/year)",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
```

This line almost goes through the origin! With no change in NO23, we expect no change in hypoxia frequency. Interesting though is the direction: with decreasing NO23, we expect to see more readings of DO<2. As Carl mentioned in a data team meeting, we expect causality here to be DO on NO23; more DO means more N can oxidize to NO23. So this is very correlative.  
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = TRUE) +
  labs(title = "Partial effect of PO4 trend on DO < 2 trend",
       x = "PO4 trend (%/year)",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
```

Again, pretty close to the origin. Expect increasing frequency of hypoxia when PO4 is increasing, and decreasing hypoxia when PO4 is decreasing.  
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = FALSE) +
  labs(title = "Partial effect of log(Chl a) median on DO < 2 trend",
       x = "long-term log(Chl a (ug/L)) median",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = FALSE) +
  labs(title = "Partial effect of Chl a median on DO < 2 trend",
       x = "long-term Chl a median (ug/L)",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
```
