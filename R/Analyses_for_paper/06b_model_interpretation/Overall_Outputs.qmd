...
output-file: "2024-08-02 Model Outputs main"
date: today
format: 
  pdf:
    toc: true
...
Standardized coefficients, as in the coefficient plots and the left columns of the coefficient tables, are interpreted on a scale of standard deviations: one standard deviation in [predictor] is expected to be associated with a [coefficient]-unit change in [response].  
...
coeffs_doLT2 <- doLT2_list$coeffs_stnd |> 
  mutate(model = "DO < 2 trend",
         sw = as.numeric(sw_all)) |> 
  select(-sw_all, -sw_nonnested)
nmods_doLT2 <- data.frame("n.models" = attributes(doLT2_list$coeffs_stnd$sw_all)$n.models) |> 
  tibble::rownames_to_column("term")
coeffs_doLT2 <- full_join(coeffs_doLT2, nmods_doLT2, by = "term")
...
transform_mults <- doLT2_list$dat_sds |> 
    pivot_longer(cols = everything(),
                 ...
                 values_to = "sd_x") |> 
    mutate(trans_mult = sd_y / sd_x)
coeffs_doLT2 = left_join(coeffs_doLT2, transform_mults) |> 
    mutate(Estimate.natural = Estimate * trans_mult,
           Adjusted.SE.natural = Adjusted.SE * trans_mult)
...
From the graph above and the table below we see 5 important predictors for DO mg/L trend: trend in daily PAR (positive association), temperature trend (negative association),  chl a trend (negative association), SpCond median (positive), and SpCond trend (negative). Chl a median (positive association) is of moderate importance. 
...
**# models in top set (delta < 4):**  `r nrow(doLT2_list$top_modsd4)`

```{r}
graph_coeffs(doLT2_list$coeffs_stnd, title_param = "\ntrend in proportion of DO readings < 2 mg/L")
```


The graph above and the table below indicate 3 major predictors for DO < 2 trend / hypoxia trend, all negatively associated with trend in hypoxia: median precipitation, NO23 trend, and daily PAR trend. SpCond trend (positive) and SpCond median (negative) are of moderate importance. Chl a trend (positive) has some indication of being important but is not as strong as the other predictors.  


```{r}
table_coeffs(coeffs_doLT2, title_param = "DO < 2 trend", delta = 4)
```
...
This slope is negative; but even across our values for NH4 trend, when you hold all other predictors equal we're still generally expecting increasing chl. However, the size of the chl increases is negativaly associated with NH4 change - the biggest increases in chl are expected to happen alongside the biggest decreases in NH4. 
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = FALSE) +
  labs(title = "Partial effect of daily total PAR trend on DO mg/L trend",
       x = "daily PAR trend (mmol/m^2 / yr)",
...
We're generally always expecting decreases in DO, and those decreases get bigger as we get bigger increases (through time) in water temperature.
...
```{r}
pred_on_resp <- make_predictions(data = dat_doLT2,
                                 predictor = "no23f_trend",
                                 avgd_mod = mod_avgd4,
                                 means = dat_means,
                                 sds = dat_sds,
                                 sd.range = c(-2.6, 3.2),
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = TRUE) +
  labs(title = "Partial effect of SpCond trend on DO < 2 trend",
       x = "NO23 trend (%/year)",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
```
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = FALSE) +
  labs(title = "Partial effect of median Precip on DO < 2 trend",
       x = "long-term median monthly Precipitation (mm)",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
```


All else being equal, we'd expect drier stations (those with with <70 mm/month of precip) to have increasing hypoxia, and wetter stations to have decreasing hypoxia.  
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = FALSE) +
  labs(title = "Partial effect of SpCond median on DO < 2 trend",
       x = "long-term median Specific Conductance (mS/cm)",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
```
...
graph_predictions(pred_on_resp,
                  response.is.log.trend = FALSE,
                  predictor.is.log.trend = TRUE) +
  labs(title = "Partial effect of Chl a trend on DO < 2 trend",
       x = "Chl a trend (%/year)",
       y = "Expected DO < 2 trend (proportion of readings / yr)")
```


With increasing chl, we expect increasing hypoxia. (and, from the DO mg/L graphs, we expect decreasing DO trends - makes sense that decreasing DO and increasing hypoxia would go together.)  
