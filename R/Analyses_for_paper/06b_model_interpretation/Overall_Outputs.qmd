...
```{r}
# make a big table of coefficients
...
# join
coeffs_all <- bind_rows(coeffs_chl, coeffs_domgl, coeffs_doLT2)

# cleanup
rm(nmods_chl,
   nmods_domgl,
   nmods_doLT2,
   coeffs_chl, coeffs_domgl, coeffs_doLT2)
```

```{r}
# clean up coeffs data frame and make a nice table
coeffs_totbl <- coeffs_all |> 
  select(model, term, Estimate, Adjusted.SE, sw, n.models) |> 
  mutate(Estimate = round(Estimate, 4),
         Adjusted.SE = round(Adjusted.SE, 4),
         sw = round(sw, 2)) |> 
  rename(SE = Adjusted.SE,
         Predictor = term) |> 
  pivot_wider(names_from = model,
              values_from = c(Estimate, SE, sw, n.models),
              names_glue = "{model}_{.value}",
              names_vary = "slowest") |> 
  arrange(desc(`Chl a trend_sw`))

gt(coeffs_totbl) |> 
  tab_spanner_delim(delim = "_") |> 
  tab_header(title = "Standardized coefficients and importance",
             subtitle = "Model averaging with delta AICc < 4") |> 
  tab_style(style = cell_borders(sides = "right",
                                 color = "gray80",
                                 weight = px(3)),
            locations = cells_body(columns = c(1, 5, 9, 13))) |> 
  opt_row_striping()
```
