...
```{r}
cat("normal chunk \n\n")
mod_chl$modelInfo$allForm$formula
```
...
```{r}
table_coeffs(coeffs_chl, response = "Chl a trend", delta = 4)
```
...
```{r}
table_coeffs(coeffs_domgl, response = "DO mg/L trend", delta = 4)
```
...
```{r}
table_coeffs(coeffs_doLT2, response = "DO < 2 trend", delta = 4)
```
