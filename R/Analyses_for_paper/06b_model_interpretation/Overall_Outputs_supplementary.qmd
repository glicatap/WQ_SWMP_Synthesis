...
This file contains information on model outputs for our three key responses, using model-averaging (full model method) on models with delta AICc < 2 and < 6 from the top model. delta < 4 will be discussed in the main paper, and this file provides information for supplementary materials.    
...
```{r}
graph_coeffs(chl_list$coeffs_stnd_d2, title_param = "Chl a trend",
             subtitle = "delta < 2")

graph_coeffs(chl_list$coeffs_stnd_d6, title_param = "Chl a trend",
             subtitle = "delta < 6")
```
...
```{r}
graph_coeffs(domgl_list$coeffs_stnd_d2, title_param = "DO mg/L trend",
             subtitle = "delta < 2") 

graph_coeffs(domgl_list$coeffs_stnd_d6, title_param = "DO mg/L trend",
             subtitle = "delta < 6")
...
```{r}
graph_coeffs(doLT2_list$coeffs_stnd_d2, 
             title_param = "\ntrend in proportion of DO readings < 2 mg/L",
             subtitle = "delta < 2")

graph_coeffs(doLT2_list$coeffs_stnd_d6, 
                   title_param = "\ntrend in proportion of DO readings < 2 mg/L",
                   subtitle = "delta < 6")
