...
Join in way that keeps only stations that are in both WQ and NUT data frames. Keep only stations that have different start/end years for both WQ and NUT (i.e. more than one year for WQ, and more than one year for NUT).   
...
dat_out <- left_join(all_meds, 
  ...
  select(station, `Station Name`,
         ...
         Latitude, Longitude,
         everything()) |> 
  arrange(chla_nMonths)
```

```{r}
write.csv(dat_out,
          here::here("Outputs",
                     "WQ-NUT_overallMedians_20240515.csv"))
```
