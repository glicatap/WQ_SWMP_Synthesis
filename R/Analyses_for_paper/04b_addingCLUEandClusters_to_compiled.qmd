...
comp_doLT2_in <- here::here("Outputs", "04_compiled_predictors",
                      "doLT2_compiled_predictors.csv")

comp_out <- here::here("Outputs", "04_compiled_predictors",
                      "compiled_predictors_withExternalInfo.csv")
comp_doLT2_out <- here::here("Outputs", "04_compiled_predictors",
                      "doLT2_compiled_predictors_withExternalInfo.csv")

# external info to import
clue_in <- here::here("Data", "Copy of SWMP CLUE_All Site Classification_Mar2022_for review.xlsx")
...
comp_doLT2 <- read.csv(comp_doLT2_in)

comp_joined <- comp |> 
  left_join(clue2, by = "station") |> 
  left_join(clusters, by = "station")

comp_doLT2_joined <- comp_doLT2 |> 
  left_join(clue2, by = "station") |> 
  left_join(clusters, by = "station")
```
