...
# What if we did just do principal components regression?  

What would those PCs look like? Using base R's `princomp` here because it automatically removes variables that don't significantly load on an axis, which should make the outputs easier to interpret.  

```{r}
chl_predictors <- chl_df |> 
  select(-chla_trend)
big_pca <- princomp(chl_predictors)
```
...
test <- big_pca$loadings


loadings(big_pca, cutoff = 0.2)
```


# PCA Predictor Attempt 4  

Some of the variables I added to the latitudinal PCA didn't load much the first several axes, so I will remove those again. Based on loadings/correlations, `prcp_trend` and `spcond_trend` need to stay together; and `temp_trend`, `temp_seasAmp_trend`, and `chl_seasAmp` need to stay together. I'm not sure I like the idea of PCAs with only 2-3 variables so may try all 5 of those together and see what happens.  

This leaves 4 variables in the main latitude PCA: `latitude`, `temp_median`, `temp_seasAmp`, and `dailyPAR_median`. Those 4 really make a lot of sense together. It's just breaking up all the rest that gets difficult.  

From the other attempted PCAs, some of the water and nutrient variables also fall out together. What I was calling the "water PCA" in attempt 3 above seemed to group things pretty well - everything loaded fairly highly onto at least one of the two axes. Teh exception might be po4f_trend but it seems to need to be with nh4f_trend, which does load more strongly. So I think that whole 'water' one can stay together.  

Looks like we should take out PC5 from the water PCA, as it correlates strongly and negatively with latitude PC1 (at least as constructed in attempt 3 - maybe not as much when I remove precip, spcond, and some temp/chl factors from the latitudinal PCA).  
...
# site scores - to add to chl predictor df before the next round
sites_lat <- as.data.frame(scores(lat_pca, choices = 1:3,
                     display = "sites",
                     scaling = 0)) |> 
  set_names(c("Lat-PC1", "Lat-PC2", "Lat-PC3"))
```
...
Want 3 of these. Although chl seasonal amplitude is really just wanting to be on its own - maybe it should be removed.        
