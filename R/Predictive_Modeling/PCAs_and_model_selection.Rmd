---
title: "Predictor PCAs and model selection"
output: 
  html_document:
    toc: true
    toc_float: true
...
# Final? PCA Predictor Combinations
...
```{r}
combined_df2 <- bind_cols(chl_df3,
                         sites_lat,
                         sites_SPT)
my_corrplot(combined_df2, start_col = 2)
mod2a <- lm(chla_trend ~ ., data = combined_df2)
performance::check_model(mod2a)
vif_graph(mod2a)
summary(mod2a)
...
### 'water' - nutrients, water and prcp medians PCA
...
```{r}
rm(chl_df, chl_df2, chl_df3, chl_df4, eigs, lat_df, lat_pca,
   mod1, mod2, scores_lat, scores_SPT, scores_water,
   sites_lat, sites_SPT, sites_water, SPT_df, SPT_pca,
   water_medians_df, water_medians_pca)
```
...
Response: chla_trend  
Predictors: all above  

`combined_df` is the data frame with PCA scores and other predictors centered & scaled.  
`responses_main` is the data frame with unaltered response variables.  

```{r ready-chl}
chl_response <- responses_main |> 
  select(station, chla_trend)


# chl_predictors <- combined_df |> 
#   select(-chla_trend, -"water-PC5", -"water-PC4") |>  # chla_trend was scaled here; get rid of 5th PC
#   rownames_to_column("station")
# chl_forMod <- left_join(chl_response, chl_predictors, by = "station") |> 
#   select(-station)


chl_predictors <- combined_df |> 
  select(-chla_trend, -"water-PC5") |>  # chla_trend was scaled here
  rownames_to_column("station")
chl_forMod <- left_join(chl_response, chl_predictors, by = "station") |> 
  select(-station)
```
...
```{r mod-chl}
chl_mod <- lm(chla_trend ~ ., data = chl_forMod)
```
...
```{r}
performance::check_normality(chl_mod)
performance::check_heteroscedasticity(chl_mod)
```
...
op <- par()
par(mfrow = c(3, 5))
preds_vec <- names(chl_forMod)
preds_vec <- preds_vec[-1]  # get rid of chla_trend
resids <- resid(chl_mod)

for(i in seq_along(preds_vec)){
  tmp <- unlist(chl_mod$model[preds_vec[i]])
  plot(resids ~ tmp, xlab = preds_vec[i])
  abline(h = 0, lty = 2)
}
...
log percent developed has a bit of funneling. The others, I'm more or less okay with.  


## Model selection  

Probably don't want to do all subsets here. Hm.  


```{r, eval = FALSE}
options(na.action = "na.fail")
mod_subsets <- MuMIn::dredge(chl_mod, eval = FALSE)
