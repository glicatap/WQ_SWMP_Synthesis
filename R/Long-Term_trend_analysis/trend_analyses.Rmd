...
for(i in 1:nrow(wq_grid)){
  ...
  if(inherits(try(run_bam_wq(tmp, k = 12), silent = TRUE), "try-error")){
    ...
    # first try fixing it by letting the model choose 'k'
    if(inherits(try(run_bam_wq(tmp, k = -1), silent = TRUE), "try-error")){
...
# save out this data frame so we don't have to run everything every time
# probably want to deal with stations that errored first, and add them in
```
...
for(i in 1:nrow(nut_grid)){
  ...
  if(inherits(try(run_bam_nut(tmp, k = 12), silent = TRUE), "try-error")){
    ...
    # first try fixing it by letting the model choose 'k'
    if(inherits(try(run_bam_nut(tmp, k = -1), silent = TRUE), "try-error")){
      # if that didn't work, make a blank row in the output:
      out <- data.frame(station = stn,
                        parameter = param,
                        model_error = TRUE)  
