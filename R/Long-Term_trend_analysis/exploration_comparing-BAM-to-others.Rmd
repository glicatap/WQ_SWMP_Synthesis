...
The default, with no arguments, doesn't account for unevenly spaced or missing data. But it gets somewhat close to the rho given by bam.  
...
dat_gam_ar <- gamm(value ~ dec_date + s(month, bs = "cc", k = 12),
                     ...
                     method = "REML")
summary(dat_gam_ar$gam)
summary(dat_gam_ar$lme)  # Phi = 0.5457
acf(resid(dat_gam_ar$lme, type = "normalized"))[1] 
acf(resid(dat_gam_ar$lme, type = "response"))[1]  # 0.524
```


Now using `~1|year`, to allow grouping within year:  
...
And `~ month | year`, because month is the time step. This seems like it's probably the most appropriate of the `gamm` options.   

Slope output is -0.4355, SE 0.1346.  
...
```{r}
dat_seas <- gam(value ~ dec_date + s(month, bs = "cc", k = 12),
                  data = dat)
summary(dat_seas)
acf(residuals(dat_seas), plot = FALSE)[1]
