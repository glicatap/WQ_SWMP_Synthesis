...
On the left is a floating table of contents for navigating between major sections of output. It moves through the three main types of SWMP data, first only the primary parameters of focus for the SWMP Synthesis project. After the primary focal nutrients have been graphed, the document cycles back to remaining water quality, weather, and nutrient parameters.    
...
```{r primary-secondary-params}
wq_primary <- c("temp", "spcond", "sal", "do-pct", "do-mgl", "turb")
wq_secondary <- c("ph", "depth", "cdepth", "level", "clevel")

met_primary <- c("atemp", "dailyPAR")
met_secondary <- c("rh", "bp", "wspd", "maxwspd", "wdir")

nut_primary <- c("chla", "nh4f", "no23f", "po4f")
nut_secondary <- c("no2f", "no23f")
```
...
dat <- wq %>% 
  ...
  select(station, year, month, ends_with("min"), ends_with("median"), ends_with("max"), ends_with("nValid")) %>% 
  remove_empty("cols") %>% 
...
```{r, results = "asis"}
parms <- unique(wq_long$param) 
parms <- parms[which(parms %in% wq_primary)]
for(i in seq_along(parms)){
  cat("\n")
  cat("##", parms[i], "\n")
  dat_sub <- filter(wq_long, param == parms[i])
...
dat <- met %>% 
  filter(reserve == params$reserve,
         station %in% stns_active) %>% 
  select(station, year, month, ends_with("min"), ends_with("median"), ends_with("max"), ends_with("total"), ends_with("nValid")) %>% 
  remove_empty("cols") %>% 
  rename(any_of(col_matching))
...
parms <- unique(met_long$param) 
parms <- parms[which(parms %in% met_primary)]  # precip treated differently, below loop

for(i in seq_along(parms)){
  cat("\n")
  cat("##", parms[i], "\n")
  
  dat_sub <- filter(met_long, param == parms[i])
  p <- ggplot(dat_sub, aes(x = date, y = value, col = stat)) +
    geom_line(linewidth = 0.3) +
    geom_point(aes(text = hoverText), # this produces a warning but trying any other way messes up the line geom
               size = 0.6) +
...
cat("\n")
cat("##", "totprcp", "\n")
dat_sub <- filter(met_long, param == "totprcp")
p <- ggplot(dat_sub, aes(x = date, y = total)) +
  geom_line(col = "navy", linewidth = 0.3) +
  geom_point(aes(text = precipText),
             col = "navy", size = 0.6) +
...
parms <- unique(nut_long$param) 
parms <- parms[which(parms %in% nut_primary)]
for(i in seq_along(parms)){
  cat("\n")
  cat("##", parms[i], "\n")
  dat_sub <- filter(nut_long, param == parms[i])
...
```{r, results = "asis"}
parms <- unique(wq_long$param) 
parms <- parms[which(parms %in% wq_secondary)]
...
```{r, results = "asis"}
parms <- unique(met_long$param) 
parms <- parms[which(parms %in% met_secondary)]
...
```{r results = "asis"}
parms <- unique(nut_long$param) 
parms <- parms[which(parms %in% nut_secondary)]
